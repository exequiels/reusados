<!-- Topbar -->
<table class="table table-sm table-bordered border-estilo">
    <tr>
        <td class="py-1 px-3">
            <div class="d-flex flex-wrap align-items-center justify-content-end">
                <!--<div class="col-12 col-sm-auto me-sm-2 mt-1 mb-1">
                    <button class="w-100 border border-dark-subtle p-2"><a href="?dir=mi-perfil">Perfil</a></button>
                </div>-->
                <div class="col-12 col-sm-auto me-sm-2 mt-1 mb-1">
                    <button class="w-100 border border-dark-subtle p-2"><a href="?dir=registrarse">Registrarse</a></button>
                </div>
                <div class="col-12 col-sm-auto me-sm-2 mt-1 mb-1">
                    <button class="w-100 border border-dark-subtle p-2"><a href="?dir=login">Login</a></button>
                </div>
            </div>
        </td>
    </tr>
</table>
<!-- // Topbar -->