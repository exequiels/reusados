...
$logFile = __DIR__ . '/cron_accesorios.txt';
...
if ($pdo) {
    $updateStmt = $pdo->prepare("UPDATE mla_consolasyvideojuegos_principal SET status = 0 WHERE status = 1 AND source = :source");
    if (!$updateStmt->execute([':source' => 'accesorios'])) {
...
function extraerDatos($pdo, $SITE_ID, $ACCESS_TOKEN, $searchTerms)
{
    ...
    foreach ($searchTerms as $category => $terms) {
        foreach ($terms as $term) {
            logMessage("Buscando término: '$term'");
            $offset = 0;
            $apiBatchSize = 50;
            $counter = 1;

            do {
                logMessage("Iniciando llamada API para '$term', offset: $offset");
                ...
                curl_close($curl);
                logMessage("Respuesta API recibida para '$term', offset: $offset");
                ...
                if ($pdo) {
                    ...
                    logMessage("Iniciando inserción de datos para '$term'");
                    ...
                    logMessage("Insertados $resultsCount artículos del término '$term'. Total acumulado: $totalInserted");
                } else {
                ...
                logMessage("Llamado No. $counter para '$term': Se han obtenido " . count($results) . " artículos");
                $counter++;
            ...
            logMessage("Búsqueda completada para el término '$term'");
        }
    }
    ...
    if ($pdo) {
        ...
        $deleteStmt = $pdo->prepare("DELETE FROM mla_consolasyvideojuegos_principal WHERE status = 0 AND source = :source");
        $deleteStmt->execute([':source' => 'accesorios']);
...
$searchTerms = [
    'MLA438578' => ['nintendo nes', 'super nintendo', 'nintendo 64', 'gamecube', 'sega genesis', 'sega saturn', 'sega dreamcast', 'playstation 1', 'playstation 2', 'playstation 3'],
];
